...
def adjust_rate(price, prev_price, ma_gradient):
    if prev_price is None:
        return 1
    ma_gradient = 1+(ma_gradient/100)
    gradient = prev_price / price
    i = 1.0
    while i <= 5.0:
        new_gradient = gradient * i
        if (new_gradient > 0.9 * ma_gradient) and (new_gradient < 1.1 * ma_gradient):
            return i
        i += 0.1
    return 1.0
...
class StockRawData():
    ...
    def __saveRawRecords(self, outFilename, cateFileName):
        ...
        for (k, fields) in self.__dict.items():
            ...
            if fund_records is None or len(fund_records) < 12:
                sys.stderr.write('lack records[' + ','.join([code, date]) + ']\n')
                continue
            hd_data = '0'
            if hd_records:
                hd_data = hd_records[9]
            out_records = [
                    code,
                    date,
                    fund_records[3],
        ...
        prev_code = None
        ma_gradient = None
        sorted_records = sorted(recordList, reverse = True)
        for idx in range(len(sorted_records)):
            line = sorted_records[idx]
            items = line.split('\t')
            code = items[0]
            price = float(items[2])
            if prev_code != code:
                rate = 1.0
                prev_price = None
            new_rate = adjust_rate(price, prev_price, ma_gradient)
            rate /= new_rate
            prev_price = price
            prev_code = code 
            items[2] = str(rate * price)
            ma_gradient = float(items[3])
            line = '\t'.join(items)
            fout.write(line + '\n')
        fout.close()
...
class FeatureData():
    ...
    def processTestData(self, inFilename, outFilename):
        ...
        for line in open(inFilename):
            ...
            features = self.__genFeature(records)
    ...
    def __generateTrainFeature_v1(self, outFilename):
        ...
        for fields in self.__records:
            item_list = self.__genFeature(fields)
    ...
    def __genFeature(self, records):
        return self.__genFeature_v2(records)
    ...
    def __genFeature_v2(self, records):
        item_list = []
        label = records[0]
        label_str = -1
        if label > 0:
        ...
        label_str = str(label_str)
        for idx in range(1, len(records)):
            desc = "%d:%f" % (idx, records[idx])
            item_list.append(desc)
        res_records = [label_str,]
        res_records.extend(['%d:%s' % (k,v) for (k,v) in self.__genValueFea(item_list)])
    ...
    def __genValueFea(self, records):
        feaList = []
        for record in records:
            k,v = record.split(':')
            kid = self.__getFeatIdx(k)
            feaList.append([kid, v],)
        return sorted(feaList, key=lambda item: item[0])
